#!/usr/bin/env python3
"""
Create lit_part_texts table and populate it from easter_propers.json.

Usage:
    python translations/load_lit_part_texts.py [--drop]
"""
...
CREATE_TABLE = """
CREATE TABLE IF NOT EXISTS lit_part_texts (
...
def classify_day(entry):
    """
    Return (subseason, wknum, wkday) given a JSON day entry.
    ...
    if week == 1:
        ...
        if weekday == 'Sunday':
            ...
            return ('AD_ASC', 2, wkday)
    elif week in (3, 4, 5):
        return ('AD_ASC', week, wkday)
    elif week == 6:
        if weekday in ('Friday', 'Saturday'):
            return ('POST_ASC', 6, wkday)
        else:
            return ('AD_ASC', 6, wkday)
    elif week == 7:
        return ('POST_ASC', 7, wkday)
    else:
        return (None, week, wkday)
...
def build_rows(entry):
    ...
    """
    subseason, wknum, wkday = classify_day(entry)
    rows = []
...
def main():
    ...
    with engine.begin() as conn:
        if args.drop:
            conn.execute(text('DROP TABLE IF EXISTS lit_part_texts'))
            print('Dropped existing lit_part_texts table.')

        conn.execute(text(CREATE_TABLE))
        print('Table lit_part_texts ready.')
        ...
        print(f'Total rows in lit_part_texts: {count}')

        sample = conn.execute(text(
            "SELECT season, subseason, wknum, wkday, service_part, text_src "
            "FROM lit_part_texts ORDER BY subseason, wknum, wkday, service_part LIMIT 10"
        )).fetchall()
        print('\nSample rows:')
        for r in sample:
